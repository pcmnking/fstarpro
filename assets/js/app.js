...
document.addEventListener('DOMContentLoaded', () => {
    ...
    let activeSourceBranch = null; // Track the specific clicked palace branch
    ...
    function drawTransformationArrows(sourceBranch) {
        ...
        activeTransStars.forEach((star, idx) => {
            ...
            const indexOffset = (idx - 1.5) * 15; // Spread arrows
    ...
    function render() {
        ...
        if (activeSourceBranch) {
            drawTransformationArrows(activeSourceBranch);
        }
        ...
        // --- AI Expert Analysis Logic ---
        if (typeof ExpertAnalyst !== 'undefined') {
            const analyst = new ExpertAnalyst(chart);
            const report = analyst.generateReport();

            const adviceContainer = document.getElementById('expert-analysis-content');
            if (adviceContainer) {
                // Formatting helper
                const formatSection = (title, content, icon) => {
                    return `
                        <div style="margin-bottom: 12px;">
                            <strong style="display:block; color:#444; margin-bottom:4px;">${icon} ${title}</strong>
                            <div style="padding-left: 0px; color:#333;">${content}</div>
                        </div>
                    `;
                };

                let adviceHtml = '';
                adviceHtml += formatSection('財運診斷 (Wealth)', report.wealth_advice, '');
                adviceHtml += formatSection('健康與劫數 (Health)', report.health_warning, '');
                adviceHtml += formatSection('感情診斷 (Relationship)', report.love_analysis, '');
        ...
        if (activeSourceBranch) {
            ...
            let analysisHtml = `<h3>【${sourcePalace.title}】四化飛伏分析</h3>`;
                    ...
                    activeTransStars.forEach((star, idx) => {
                        ...
                        if (isZiHua) {
                            ...
                            if (typeof ZIWEI_DATA_ZIHUA !== 'undefined' && ZIWEI_DATA_ZIHUA[sourceName] && ZIWEI_DATA_ZIHUA[sourceName][type]) {
                                ...
                                // Fallback if key + "宮" logic needed (though ZIWEI_DATA_ZIHUA seems to use exact keys based on user diff)
            ...
            ui.analysisContainer.innerHTML = `<h3>宮位四化分析</h3><p style="color:#666;">點擊盤面天干查看詳情...</p>`;
        }
    ...
    ui.resetBtn.addEventListener('click', () => {
        ...
        ui.zuofuPos.value = '辰';
        ui.youbiPos.value = '戌';
        activeSourceBranch = null; // Reset selection
        render();
    });
    ...
    ui.chartContainer.addEventListener('click', (e) => {
        ...
        if (e.target.classList.contains('celestial')) {
            ...
            if (palaceDiv) {
                ...
                if (activeSourceBranch === branch) {
                    activeSourceBranch = null;
                } else {
                    activeSourceBranch = branch;
                }
                render();
            }
        }
